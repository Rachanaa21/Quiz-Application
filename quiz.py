    ...
    def addUserToDataBase():
        ...
        if len(fname.get())==0 and len(user.get())==0 and len(pas.get())==0 and len(c.get())==0:
            ...
            conn = psycopg2.connect(
            		...
            		port = 8080)
            cur = conn.cursor()
            create_script = '''CREATE TABLE IF NOT EXISTS userSignUp(FULLNAME text, USERNAME text,PASSWORD text,COUNTRY
            text)'''
            cur.execute(create_script)
            cur.execute("INSERT INTO userSignUp VALUES (?,?,?,?)",(fullname,username,password,country)) 
            conn.commit()
            cur.execute('SELECT * FROM userSignUp')
            z=cur.fetchall()
            print(z)
            cur.close()
            conn.close()
...
def start():
    ...
    canvas.create_image(720,440,image=img,anchor=NW)
